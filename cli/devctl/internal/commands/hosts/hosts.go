...
func handle(ctx *cmdregistry.Context) error {
	...
	if !config.HasRuntimeFlake(cfg) {
		return fmt.Errorf("hosts requires runtime.flake")
	}
...
func parseOptions(args []string) (options, error) {
	...
	if out.mode != "print" && out.mode != "apply" && out.mode != "check" {
		return out, fmt.Errorf("usage: hosts [print|apply|check] [--target host]")
	}
	if out.target != "host" {
		return out, fmt.Errorf("native hosts supports only --target host")
	}
	return out, nil
}
