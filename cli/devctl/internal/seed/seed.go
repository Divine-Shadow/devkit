...
func BuildDirectHomeScripts(home string, seedCodex bool) []string {
	...
	parts := []string{
		...
		"if [ \"$dev_home_ok\" = 1 ] && [ -n \"${DOCKER_HOST:-}\" ]; then docker_api_version=\"${DOCKER_API_VERSION:-1.40}\"; printf \"docker.host=%s\\n\" \"$DOCKER_HOST\" > \"$home/.testcontainers.properties\"; printf \"DOCKER_HOST=%s\\napi.version=%s\\n\" \"$DOCKER_HOST\" \"$docker_api_version\" > \"$home/.docker-java.properties\"; ln -sfn \"$home/.testcontainers.properties\" /home/dev/.testcontainers.properties; ln -sfn \"$home/.docker-java.properties\" /home/dev/.docker-java.properties; fi",
