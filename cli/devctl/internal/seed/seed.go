...
func BuildAnchorScripts(cfg AnchorConfig) []string {
	...
	parts := []string{
		...
		"if [ \"$dev_home_ok\" = 1 ] && [ -n \"${DOCKER_HOST:-}\" ]; then docker_api_version=\"${DOCKER_API_VERSION:-1.52}\"; printf \"docker.host=%s\\n\" \"$DOCKER_HOST\" > \"$target/.testcontainers.properties\"; printf \"DOCKER_HOST=%s\\ndocker.host=%s\\napi.version=%s\\ndocker.api.version=%s\\n\" \"$DOCKER_HOST\" \"$DOCKER_HOST\" \"$docker_api_version\" \"$docker_api_version\" > \"$target/.docker-java.properties\"; ln -sfn \"$target/.testcontainers.properties\" /home/dev/.testcontainers.properties; ln -sfn \"$target/.docker-java.properties\" /home/dev/.docker-java.properties; fi",
...
func BuildDirectHomeScripts(home string, seedCodex bool) []string {
	...
	parts := []string{
		...
		"if [ \"$dev_home_ok\" = 1 ] && [ -n \"${DOCKER_HOST:-}\" ]; then docker_api_version=\"${DOCKER_API_VERSION:-1.52}\"; printf \"docker.host=%s\\n\" \"$DOCKER_HOST\" > \"$home/.testcontainers.properties\"; printf \"DOCKER_HOST=%s\\ndocker.host=%s\\napi.version=%s\\ndocker.api.version=%s\\n\" \"$DOCKER_HOST\" \"$DOCKER_HOST\" \"$docker_api_version\" \"$docker_api_version\" > \"$home/.docker-java.properties\"; ln -sfn \"$home/.testcontainers.properties\" /home/dev/.testcontainers.properties; ln -sfn \"$home/.docker-java.properties\" /home/dev/.docker-java.properties; fi",
