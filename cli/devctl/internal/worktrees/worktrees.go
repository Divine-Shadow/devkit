...
func run(dry bool, name string, args ...string) error {
    ...
    if res.Code != 0 { return fmt.Errorf("%s %v: exit %d", name, args, res.Code) }
    return nil
}
...
func Setup(devkitRoot, repo string, n int, baseBranch, branchPrefix string, dry bool) error {
    ...
    envGit := func(args ...string) []string { return append([]string{"-u", "GIT_SSH_COMMAND", "git", "-c", "core.sshCommand=ssh"}, args...) }

    if err := run(dry, "env", envGit("-C", repoPath, "fetch", "--all", "--prune")...); err != nil { return err }
    if err := run(dry, "env", envGit("-C", repoPath, "config", "push.default", "upstream")...); err != nil { return err }
    if err := run(dry, "env", envGit("-C", repoPath, "config", "worktree.useRelativePaths", "true")...); err != nil { return err }
    // agent1 uses primary path
    b1 := fmt.Sprintf("%s1", branchPrefix)
    if err := run(dry, "env", envGit("-C", repoPath, "checkout", "-B", b1)...); err != nil { return err }
    if err := run(dry, "env", envGit("-C", repoPath, "branch", "--set-upstream-to=origin/"+baseBranch, b1)...); err != nil { return err }

    for i := 2; i <= n; i++ {
        parent := filepath.Join(devRoot, fmt.Sprintf("%s%d", branchPrefix, i))
        if !dry { _ = os.MkdirAll(parent, 0755) }
        ...
        if err := run(dry, "env", envGit("-C", repoPath, "worktree", "add", wt, "-B", bi, "origin/"+baseBranch)...); err != nil { return err }
        if !dry { rewriteGitdir(wt) }
        if err := run(dry, "env", envGit("-C", wt, "branch", "--set-upstream-to=origin/"+baseBranch, bi)...); err != nil { return err }
    }
    return nil
}
